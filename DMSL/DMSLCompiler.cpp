...
void Dmsl::Compiler::PopOprStack(stack<Dmsl::Compiler::ParsedMathWord::Data::Opr>& stk,vector<uint8_t>& out,std::stack<int>& testStk,const Precompiled& in){
    ...
    case ParsedMathWord::Data::Opr::CALL_FUNC:{
            ...
            PushAddress(stk.top().funcAddress,out);break;
...
bool Dmsl::Compiler::Compile(const Precompiled & in, Compiled & out,ostream& log)
{
    ...
    out.metParamNum = in.metParamNum;
    out.funcParamNum = in.funcParmNum;

    out.uniformSize = in.uniformVars.size();
    out.cFuncSize = in.cFuncs.size();
    out.cMetSize = in.cMet.size();

    out.linkTable.clear();
    for(auto& p:in.attributeVars) out.linkTable[p.first] = p.second;
    ...
    for(auto& p:in.cFuncs){
        if(out.linkTable.count(p.first) == 1) log<<"函数 "<<p.first<<" 与已有的变量重名。"<<endl;
        out.linkTable[p.first] = p.second;
    }
    for(auto& p:in.cMet){
        if(out.linkTable.count(p.first) == 1) log<<"方法 "<<p.first<<" 与已有的变量或函数重名。"<<endl;
        out.linkTable[p.first] = p.second;
    }
    return success;
}

void Dmsl::Compiler::CompileUnit(const Dmsl::Compiler::Precompiled& in,const Precompiled::Unit& unit,std::vector<uint8_t>& out,const std::vector<std::string>& inProgram){
    #ifdef _SHOW_ASM
...
uint32_t Dmsl::Compiler::SearchSameLayerBlockCode(const std::vector<std::string>& code,const std::string& leftWord,uint32_t begin){
    int layer = 0;
    for(;;++begin){
        //cout<<endl<<"SEARCH:"<<layer<<" "<<code[begin];
        if(layer == 0){
            if(ReadLeftWord(code[begin]) == leftWord) return begin;
        };
        if(ReadLeftWord(code[begin]) == "if" || ReadLeftWord(code[begin]) == "while" ||ReadLeftWord(code[begin]) == "for" )
            ++layer;
        if(ReadLeftWord(code[begin]) == "end"){
...
void Dmsl::Compiler::CompileLines(const Dmsl::Compiler::Precompiled& in,const Precompiled::Unit& unit,std::vector<uint8_t>& out,const std::vector<std::string>& inProgram,const uint32_t begin,const uint32_t end){
    for(uint32_t code_num = begin;code_num < end;++code_num){
        #ifdef _SHOW_ASM
        cout<<endl<<">>"<<inProgram[code_num];
        #endif // _SHOW_ASM
        const string& s = inProgram[code_num];
        string left = TrimAll(ReadLeftWord(s));
        string right = TrimRight(ReadRightLine(s));

        int leftType = WhatsTheVar(in,unit,left);
        if(leftType == 1 || leftType == 2){
            if(right[0] == '<' && right[1] == '-'){ //多重赋值意图
                auto floats = TrimAll(right.substr(2,right.length()-2));
                vector<string> floatStrs;
                Split(floats,floatStrs,':');
                ...
                PushAddress(floatStrs.size(),out);
                for(auto& s:floatStrs)
                    PushFloat(ToFloat(s),out);
            ...
            if(left == "for"){  //FOR意图
                ...
                Split(right,vec,',');
                if(vec.size() != 3) throw runtime_error("for语句必须有两个逗号和三个子语句。");
                init = TrimRight(vec[0]);
                test = TrimAll(vec[1]);
                step = TrimRight(vec[2]);
...
void Dmsl::Compiler::PushFloat(float a, std::vector<uint8_t>& out)
{
...
uint32_t Dmsl::Compiler::PushAddress(uint32_t a, std::vector<uint8_t>& out)
{
    uint32_t addressAddress = out.size();
...
void Dmsl::Compiler::ResetAddress(uint32_t addressSaddress,uint32_t address, std::vector<uint8_t>& out){
...
int Dmsl::Compiler::ParseMath(const Precompiled & in,const Precompiled::Unit& unit, std::string  s, std::vector<uint8_t>& out)
{
    ...
    #endif // _SHOW_ASM
    stack<int> testStk;   //0为bool，1为float
    ...
    while(1){
        ...
        if(isdigit(token[0])){  //读取到数字
            ...
            word.data.number = ToFloat(token);
        ...
        if(isalpha(token[0])){  //读到英文
            ...
            else if(WhatsTheVar(in,unit,token) >= 1 && WhatsTheVar(in,unit,token) <= 4){   //否则是量
        ...
        if(token == "+"){
            ...
            nextSubIsNega = true;
        }else throw invalid_argument("错误的表达式。");
        ParseOperator(in,word,out,oprStk,testStk);
    }
...
void Dmsl::Compiler::ParseData(const Precompiled & in,const Precompiled::Unit& unit,const ParsedMathWord & s, std::vector<uint8_t>& out,std::stack<int>& testStk)
{
	...
	if (s.type == ParsedMathWord::NUMBER) {
		...
		if(s.nega) PushFloat(-s.data.number, out);
		else PushFloat(s.data.number, out);
	}
		...
		case 4:
			PushCmd(VirtualMachine::ElfCode::PUSH, out);
			if(s.nega) PushFloat(-in.constants.at(s.data.varName), out);
			else PushFloat(in.constants.at(s.data.varName), out);
...
void Dmsl::Compiler::CompileIfLines(const Dmsl::Compiler::Precompiled& in, const Precompiled::Unit& unit, std::vector<uint8_t>& out, const std::vector<std::string>& inProgram, uint32_t begin, uint32_t end)
{
    vector<string> subProgram;

    //cout<<"IF:"<<TrimRight(ReadRightLine(inProgram[begin]))<<endl;
    ParseMath(in,unit,TrimRight(ReadRightLine(inProgram[begin])),out);
    PushCmd(Dmsl::VirtualMachine::ElfCode::IF,out);
    ...
    if(ReadLeftWord(TrimRight(inProgram[newEnd])) == "else"){
        auto line = ReadRightLine(TrimRight(inProgram[newEnd]));
        bool isSubIf = false;
        if(TrimAll(ReadLeftWord(TrimRight(line))) == "if"){
            subProgram.push_back(TrimRight(line));
            isSubIf = true;
        }
        ...
        if(isSubIf) subProgram.push_back("end");
